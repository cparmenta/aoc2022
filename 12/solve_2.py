...
def can_jump(current_level, next_level):
    return current_level - 1 <= next_level
...
def update_plot(force=False):
    ...
    if i_plot < PLOT_EVERY and not force:
        ...
        plt.colorbar()
        plt.text(S_pos[1], cum_path_matrix.shape[0] - S_pos[0]-1, 'S', color='black')
        plt.text(E_pos[1], cum_path_matrix.shape[0] - E_pos[0]-1, 'E', color='red')
        plt.text(min_A[1], cum_path_matrix.shape[0] - min_A[0]-1, 'A', color='white')
        plt.title(f'Min. distance = {int(cum_path_matrix[min_A]) if cum_path_matrix[min_A] != np.inf else "inf"}')
        plt.gcf().canvas.draw()
...
while uncompleted_neighbors or uncompleted_nodes:
    ...
    if level_matrix[current_pos] == 0:
        if cum_path_matrix[current_pos] <= cum_path_matrix[min_A]:
            min_A = current_pos
...
update_plot(True)
print(f'Length of shortest path to \'a\': {int(cum_path_matrix[min_A])}')
